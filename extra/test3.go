...
func main(){
	...
	//
	//}
	//fmt.Printf("%T",string(prod[0]))



	//fmt.Printf("%T",string(byte2))
	for i:=0;i<len(prod);i++{
	...
	tt:=product{}
	db.Where("username=?","Rishabh").First(&tt)
	//errq:=db.Find(&prod,"username=?","Rishabh")
	//fmt.Printf("%T", (errq.Get("username")))


	fmt.Printf("%T",tt.Declare)
